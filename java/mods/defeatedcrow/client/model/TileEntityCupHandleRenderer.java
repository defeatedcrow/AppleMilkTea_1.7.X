...
public class TileEntityCupHandleRenderer extends TileEntitySpecialRenderer
{
    ...
    private static final ResourceLocation SummerTex = new ResourceLocation("defeatedcrow:textures/blocks/blueglass.png");
    ...
    {
        this.setRotation(par1TileCupHandle, (float)par2, (float)par4, (float)par6, par1TileCupHandle.getDirectionByte());
    }
    ...
    public void setRotation(TileCupHandle tile, float par1, float par2, float par3, byte par4)
    {
        ...
        if (DCsConfig.useSummerRender && !DCsConfig.useJapaneseCup)
        {
        	...
        	byte l = (byte)tile.getDirectionByte();
            float j = 0;
            if (l == 0) j = 180.0F;
            if (l == 1) j = -90.0F;
            if (l == 2) j = 0.0F;
