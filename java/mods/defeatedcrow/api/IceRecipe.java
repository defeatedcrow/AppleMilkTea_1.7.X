...
public class IceRecipe {
	...
	private static int getRecipeID(ItemStack input)
	{
		...
		else
		{
			for(Entry<ItemStack, Integer> check : recipeID.entrySet())
	        {
				...
				if(input.getItem() == target.getItem() && (target.getItemDamage() == WILDCARD_VALUE || input.getItemDamage() == target.getItemDamage()))
                {
	...
	public static int getID(ItemStack input)
	{
		...
		{
			ItemStack inputs = new ItemStack(input.getItem(), 1, input.getItemDamage());
